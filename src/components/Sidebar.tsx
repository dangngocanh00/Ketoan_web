...
import { useMissingBillsBadgeCount } from '../domain/missingBillsBadge'
...
export default function Sidebar({ activePage, onNavigate }: Props) {
  ...
  // Live workload count (never a notification/unread count — see
  // missingBillsBadge.ts) for the "Bill thiếu" nav item, scoped per role.
  const missingBillsBadge = useMissingBillsBadgeCount()

  if (!currentUser) return null

  const { business: rawBusiness, system } = usesAdminUI(currentUser.role) ? navForAdmin() : navForCs()
  const business = rawBusiness.map(item =>
    item.id === 'missing-bills' ? { ...item, badge: missingBillsBadge > 0 ? missingBillsBadge : undefined } : item,
  )
